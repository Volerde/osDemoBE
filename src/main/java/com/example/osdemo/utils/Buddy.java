...
import com.example.osdemo.utils.Task;
...
//    private static final int memsize = 1024;//内存的页数
    private static final int memsize = 512;//内存的页数
...
//        Mem.put(1024, size1024K);
    ...
    //刷新内存
    static void refresh_memory(int begin, int end, int op) {
        if (op == 1) {
...
//        if (size > 1024)//大于1024的页面不存在无法分割
    ...
    public static boolean request_mem(int size, int location, com.example.osdemo.utils.Memory.Zone tmp) {
        boolean rt;
        init_mem();
...
//        com.example.osdemo.utils.Memory.Zone zone = new Memory.Zone(512-get[0], get[0]);
//        com.example.osdemo.utils.Memory.getZone().add(location + 1, zone);
//        tmp.size = size;
//        tmp.isFree = false;
        int index = com.example.osdemo.utils.Memory.getIndex();
        BDModel model = new BDModel(index,get[0],get[1],false);
        com.example.osdemo.utils.Memory.setIndex(index+1);
        com.example.osdemo.utils.Memory.getStartend().add(index,model);
        System.out.println("分配得到内存空间从" + get[0] + "==>" + get[1] + " 的 " + (get[1] - get[0] + 1) + "KB内存");
    ...
    public static boolean release_mem(Task task)//释放内存  task是要释放自己占用内存的进程
    {
        ...
        System.out.println("\n\n\n ||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||进程：" + task.task_ID + "释放地址" + rlease_mem[0] + "==>" + rlease_mem[1] + "的内存" + "大小为：" + size + "KB");
    ...
    public static boolean request_mem(Task task)//请求调页
    {
        ...
        System.out.println("     进程:" + task.task_ID + " 开始请求内存，所需内存大小==" + size + "KB");
...
//                    System.out.println(" request memory success" + "    进程需要运行时间：" + tem.running_time);
                    show_memory();
                    break;   //同一个时刻只能有一个进程获得内存，如果已经有一个进程成功分配内存，则终止循环

                } else {    //如果请求内存失败，则从输出请求失败信息，并进入下一个循环，取出进程队列中的下个进程
                    System.out.println("request memory failed");
                }
...
//                    System.out.println("    release memory success");
